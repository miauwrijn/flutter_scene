...
* `.fmat` materials may declare `depth_offset: true`, which adds a `depth_offset` field to `MaterialInputs` — how far along the view ray the shaded surface really is, in metres away from the camera — and writes it as the fragment's depth. It is what gives parallax occlusion mapping a silhouette: the marched hit occludes and is occluded where the relief is, so a brick wall's corner interleaves instead of meeting in a razor line, rather than being shaded relief on a flat polygon. The frame's projection depth row rides in `FragInfo` (`depth_projection`, read through the new `WindowDepthAlongView`/`WindowDepthAtViewDepth` helpers in `material_scene_inputs.glsl`), taken from the projection matrix in use so an orthographic lens needs no special case. Opt-in per material, because a shader that writes depth gives up early-depth rejection for every surface drawn with it; a pass that publishes no projection leaves the interpolated depth alone.
